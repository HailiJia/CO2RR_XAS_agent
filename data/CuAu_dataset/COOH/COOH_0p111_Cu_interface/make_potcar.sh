...
for el in "${elements[@]}"; do
  case "$el" in Cu) pot="Cu";; Au) pot="Au";; C) pot="C";; O) pot="O";; H) pot="H";; *) echo "ERROR: No POTCAR mapping defined for element: $el" >&2; exit 1;; esac
  src="$POTCAR_DIR/$pot/POTCAR"; [ -f "$src" ] || { echo "ERROR: Missing POTCAR: $src" >&2; exit 1; }; cat "$src" >> POTCAR
done
