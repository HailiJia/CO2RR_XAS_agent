...
class CatalystMetadata(BaseModel):
    """Catalyst metadata for downstream XAS ML feature generation."""

    elements: List[str] = Field(default_factory=list)
    composition: Optional[str] = None
    surface_facet: Optional[str] = None
    site_type: Optional[str] = None
    structure_id: Optional[str] = None


class AdsorbateMetadata(BaseModel):
    """Adsorbate/intermediate metadata for downstream XAS ML feature generation."""

    identity: Optional[str] = None
    formula: Optional[str] = None
    intermediate_class: Optional[str] = None
    binding_mode: Optional[str] = None
    adsorption_site: Optional[str] = None
    binding_atom: Optional[str] = None
...
class XASPlan(BaseModel):
    ...
    catalyst: Optional[CatalystMetadata] = Field(default=None, description="Structured catalyst metadata for XAS ML records.")
    ...
    def to_parsed_intent(self, output_dir: str) -> ParsedIntent:
        ...
        if self.catalyst is not None:
            catalyst_data = self.catalyst.model_dump(exclude_none=True) if hasattr(self.catalyst, "model_dump") else self.catalyst.dict(exclude_none=True)
            structure_id = catalyst_data.pop("structure_id", None)
            metadata_overrides["catalyst"] = catalyst_data
            if structure_id:
                metadata_overrides["structure_id"] = structure_id
        if self.adsorbate is not None:
            ads_data = self.adsorbate.model_dump(exclude_none=True) if hasattr(self.adsorbate, "model_dump") else self.adsorbate.dict(exclude_none=True)
            metadata_overrides["adsorbate"] = ads_data
