...
WEB_APP_UPDATE_TAG = "v27_2026-06-30_safe_remote_ls_delete_chat_paths"
...
def relaxation_settings_from_params(params):
    ...
    return {
        ...
        "ncore": params.get("relax_ncore", 4),
...
def patch_submit_to_make_potcar(submit_path):
    """Ensure VASP submit scripts build POTCAR before VASP and stop safely if it fails.

    The generated submit script is intentionally self-contained because only one
    shell script is submitted to Slurm/SF API.  It still calls make_potcar.sh
    from inside the job directory before ``srun vasp_std``.  We keep the final
    ISAAC metadata block by setting EXIT_CODE instead of using ``set -e``.
    """
    path = Path(submit_path)
    if not path.exists():
        return
    text = path.read_text()
    if "# Build POTCAR before launching VASP" in text:
        return

    potcar_block = """# Build POTCAR before launching VASP.
# Only submit_relax.sh is submitted, so it must call make_potcar.sh itself.
if [ ! -s POTCAR ]; then
...
if [ "${EXIT_CODE}" -eq 0 ]; then
  ...
  echo 'Skipping VASP because POTCAR generation failed with exit code '${EXIT_CODE}
    ...
    if "srun vasp_std > vasp.out 2>&1 || EXIT_CODE=$?" in text:
        text = text.replace("srun vasp_std > vasp.out 2>&1 || EXIT_CODE=$?", potcar_block)
    elif "srun vasp_std > vasp.out" in text:
        text = text.replace("srun vasp_std > vasp.out", potcar_block)
    else:
...
record = {
    ...
    'generator': {
        ...
        'version': 'v15_2026-06-30_submit_scripts_write_isaac_draft_records',
...
def _remote_dir_from_natural_language(text, params):
    ...
    # Examples: "under test03", "to test03", "folder batch01",
    # "remote dir /pscratch/...".
    ...
    under_match = re.search(r"(?:under|run(?: name)?|named)\s+([A-Za-z0-9_.-]+)", raw, flags=re.I)
    if under_match:
        name = under_match.group(1).strip().strip(".,;:)")
        if name and "/" not in name:
            return f"{DEFAULT_NERSC_REMOTE_BASE}/web_xas_agent_runs/{name}"

    name_match = re.search(r"(?:folder|dir|directory|to)\s+([A-Za-z0-9_.-]+)", raw, flags=re.I)
    if name_match:
        name = name_match.group(1).strip().strip(".,;:)")
        if name.lower() in {"nersc", "perlmutter"}:
            # Not a run name; keep looking/fall back to configured path.
            pass
        elif name and "/" not in name:
            return f"{DEFAULT_NERSC_REMOTE_BASE}/web_xas_agent_runs/{name}"

    # ALCF planner may return only {"remote_run_name": "test03"}.
    # Treat a bare safe token as a run-name under the standard CO2RR scratch root.
    if re.fullmatch(r"[A-Za-z0-9_.-]+", raw) and raw.lower() not in {"", "none", "latest"}:
...
def _remote_dir_explicitly_mentioned(text, params):
    ...
    if abs_match:
        return abs_match.group(1).rstrip(".,;:)>]")
    if re.search(r"\b(under|folder|dir|directory|run name|named)\b", raw, flags=re.I):
...
def _alcf_plan_chat_intent(message, params):
    """Optional Argonne ALCF inference planner for the chat console.

    This calls the ALCF Inference Endpoints through their OpenAI-compatible
    chat-completions API. The model only returns a JSON plan; local Python still
    performs all structure generation, NERSC upload, submit, monitor, download,
    and ISAAC conversion actions.
    """
    try:
        from openai import OpenAI
    except Exception:
        return {"intent": "unknown", "explanation": "Install the client with: pip install openai globus_sdk"}
    ...
    system_prompt = (
        ...
        "list_jobs, list_remote_folder, preview_file, download_outputs, convert_isaac, "
        ...
        "For 'what files are in that folder', use list_remote_folder. "
        ...
        "Allowed JSON fields: intent, settings, pathway_adsorbates, remote_run_name, preview_file, explanation. "
...
def _execute_chat_intent(intent, text, params, uploaded_file=None, plan=None):
    ...
    if intent == "check_slurm":
        client = _chat_client_from_params(params)
        job_id = str(params.get("nersc_job_id", "")).strip()
        if not job_id:
            raise ValueError("No Slurm job ID is set.")
        result = client.slurm_status(job_id, timeout_s=90)
        st.session_state["nersc_last_slurm"] = result
        return "Slurm status checked:\n\n" + str(result.get("output", json.dumps(result, indent=2)))[:3000]

    if intent == "list_jobs":
        client = _chat_client_from_params(params)
        result = client.list_user_jobs(username=None, timeout_s=90)
        st.session_state["nersc_user_jobs"] = result
        return "Current/recent NERSC jobs:\n\n" + str(result.get("output", json.dumps(result, indent=2)))[:4000]

    if intent == "list_remote_folder":
        client = _chat_client_from_params(params)
        explicit_remote = plan.get("remote_dir") if isinstance(plan, dict) else None
        explicit_remote = explicit_remote or _remote_dir_explicitly_mentioned(text, params)
        remote_dir = explicit_remote or params.get("nersc_preview_dir") or params.get("nersc_last_submit_dir") or params.get("nersc_download_remote_dir") or params.get("nersc_remote_dir")
        params["nersc_preview_dir"] = remote_dir
        result = client.remote_ls(remote_dir, timeout_s=90, max_depth=2)
...
def _classify_chat_intent_rule_based(message):
    ...
    if any(p in low for p in ["what is the full path", "uploaded folder", "upload path", "submitted folder", "remote path", "where did", "where is"]):
        return "show_paths"
    if ("what files" in low or "which files" in low or "files are in" in low or "what's under" in low or "what is under" in low or "show me what's under" in low or "ls " in low or low.startswith("ls")) and ("folder" in low or "remote" in low or "there" in low or "that" in low or "nersc" in low or "/pscratch/" in low or "/global/" in low):
        return "list_remote_folder"
...
def _run_chat_command(message, params, uploaded_file=None):
    ...
    if params.get("chat_backend") == "Argonne ALCF":
    ...
    if isinstance(plan, dict):
        ...
        low_text = text.lower()
        if re.search(r"\b(remove|delete|rm)\b", low_text) and ("/pscratch/" in low_text or "/global/" in low_text or "that folder" in low_text or "remote" in low_text):
...
with chat_cols[0]:
    p["chat_backend"] = st.selectbox(
        "Chat backend",
        ["Rule-based", "Argonne ALCF"],
        index=safe_index(["Rule-based", "Argonne ALCF"], p.get("chat_backend", "Rule-based")),
        help="Rule-based is local/deterministic. Argonne ALCF uses ALCF Inference Endpoints to plan commands; local code executes the action.",
    )
with chat_cols[1]:
...
with chat_cols[3]:
    if p.get("chat_backend") == "Argonne ALCF":
        ...
        if token_ready or token_helper_ready:
            st.success("Argonne ALCF planner enabled for chat intent parsing. Local Python executes the workflow actions.")
        else:
            st.warning("Authenticate first: download inference_auth_token.py and run `python inference_auth_token.py authenticate`, or set ALCF_INFERENCE_ACCESS_TOKEN.")

with chat_cols[3]:
    p["chat_debug_planner"] = st.checkbox(
        "Show planner trace",
        value=bool(p.get("chat_debug_planner", False)),
        help="Show whether Argonne ALCF or rule-based planning handled the last chat command, plus the resolved intent.",
    )
...
if st.session_state.get("last_structure") is not None:
    ...
    with r3:
        ...
        p["relax_ncore"] = st.number_input("NCORE", min_value=1, max_value=32, value=int(p.get("relax_ncore", 4)))
